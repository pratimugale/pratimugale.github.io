...
2. $$x \in \mathbb{R}^n$$ i.e. x is a point in n-dimensional space, where each coordinate is a real number. <br>
We can hence also write x = [$$x_1, ... x_n$$], where $$x_1, ... x_n \in \mathbb{R}$$
...
To visualize this algorithm, I implemented this original simplex method in Python [here](https://github.com/pratimugale/derivative-free-optimization). Following is a visualization of how the algorithm works when there exists one variable. The function takes one variable (x) and is defined by F(x) = $$4x^4 - 5x^3 - 2x^2 + 2x$$. The two points (n=1; we need n+1 points) are the two points that are moving in each iteration in the following video. In the case of two points, the (n-1) simplex is simply the point itself.
